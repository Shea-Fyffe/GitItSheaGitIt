...
fuzzy.match <- function(x, y = NULL, threshold = 2.5, ...){
  ...
  if(!is.null(y) & !is.character(y)) stop("Please define y as a character vector")
  if(anyNA(x)){
    x <- na.omit(x)
  }
  x <- gsub("[[:punct:]]", "", x)
  if(object.size(x) > 500 * 1000) stop("x is too large, please subsection the vector")
  if(is.null(y)) {
    y <- unique(x)
  } else {
  y <- gsub("[[:punct:]]", "", y)
  if(any(duplicated(y))) {
    ...
    if( object.size(y) > 500 * 1000) stop("y is too large, please subsection the vector")
    y <- unique(y)
   }
  }
  x <- tolower(x)
  ...
  if(length(setdiff(x,y))==0) {
    dist <- as.matrix(stringdist::stringdistmatrix(x, method = 'osa', weight = c(1,1,1,1), useNames = "string" ))
  } else {
  xm <- x[!x %in% y]
  if (any(nchar(xm) < 5)){
    xms <- xm[nchar(xm) < 5]
...
save.xlsx <- function (file = getwd(), ...) {
  ...
  for (i in 1:nobjects) {
    if (i == 1)
      xlsx::write.xlsx(as.data.frame(objects[[i]]), file, sheetName = objnames[i],row.names = FALSE)
    else xlsx::write.xlsx(as.data.frame(objects[[i]]), file, sheetName = objnames[i], row.names = FALSE,
                          append = TRUE)
  }
