...
get_data <-  function(base_url, per_page, pn) {
  ...
  df_list <-
    lapply(paste0(base_url, "&t=a&pp=", per_page, "&o=&p=", 1:pn),
...
add_new_columns <- function(data = df) {
  ...
  data$Decision <-
    ifelse(nchar(gsub(" via.*", "", data$Decision_AND_Date)) > 11,
           "Other",
           gsub(" via.*", "", data$Decision_AND_Date))
  
  #extract and consolidate decision date values
  data$Decision_Date <-
    ifelse(
      grepl(".*on ", data$Decision_AND_Date) == TRUE,
      gsub(".*on | Undergrad.*", "", data$Decision_AND_Date),
      data$Date_Added
    )
  data$Decision_Date <-
    as.Date(data$Decision_Date, format = "%d %b %Y")
  ...
  data$Program <-
    gsub("*$\\s*\\([^\\)]+\\)", "", data$Program_AND_Season) #remove data within parenthsis
  #Validate Masters or Phd Find, otherwise OTHER
  data$Degree <-
    ifelse(
      gsub(".*, *(.*?) * [(].*$", "\\1" , data$Program_AND_Season) %in% c("PhD", "Masters"),
      gsub(".*, *(.*?) * [(].*$", "\\1" , data$Program_AND_Season),
      "Other"
    )
  
  data$Program <-
    gsub("(.*),.*", "\\1", data$Program) #Remove Data after the last Comma
  ...
  data$GRE_Verbal[(is.na(data$GRE_Verbal) |
                     data$GRE_Verbal == "n/a")] <- "0"
  data$GRE_Verbal <- as.numeric(data$GRE_Verbal)
  
  data$GRE_Quant <- sapply(strsplit(data$GRE, "/"), "[", 2)
  data$GRE_Quant[(is.na(data$GRE_Quant) | data$GRE_Quant == "n/a")] <-
    "0"
...
clean_encoding <- function(data = df) {
  data <-
    as.data.frame(sapply(data, function(x)
      iconv(x, "UTF-8", "ASCII", sub = "")), stringsAsFactors = FALSE)
  df[grep("^[^[:alnum:](]", df[, 1]), ]
  return(data)
}
...
fuzzy_match <- function(df, x, y) {
  ...
  df[, x] <-
    ifelse(is.na(match(df[, x], dist$Value)) , df[, x], dist$Index[match(df[, x], dist$Value)])
...
tt1 <- ttheme_minimal(
  core = list(
    bg_params = list(fill = c("gray92", "cornflowerblue"), col = NA),
...
set_up_table <- function(x, tt1) {
  if (names(x)[2] == "Month_Day") {
    ...
    w <- grobWidth(table)
    title <-
      textGrob(paste("Top 10 ", x[1, 1], " Dates", sep = ""), gp = gpar(fontsize =
                                                                          14))
  }
  else {
    table <- tableGrob(x, theme = tt1, rows = NULL)
    ...
    w <- grobWidth(table)
    title <-
      textGrob(paste("Top 10 ", x[1, 1], " Schools", sep = ""), gp = gpar(fontsize =
                                                                            14))
  }
