...
stat.stratifiedsample <- function(x, size = 2500, ...) {
  if(!inherits(x, "data.frame")) stop("verify x is a data.frame")
  if(is.missing(size)||!is.integer(size)||size < 0) stop("verify size is a valid, positive integer")
  strata <- list(...)
  if(length(strata)==1) {
  ...
  if(!all(strata %in% names(x))) stop(sprintf("%s is invalid try a different column name", strata[!strata %in% names(x)]))
  N <- nrow(x)
  if(size > N) {
    ...
    strat_loop <- function(x, y) {
      if(class(x)=="list") x <- as.data.frame(x)
      if(nrow(x) > y){
